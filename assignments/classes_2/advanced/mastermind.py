...
class Mastermind:
    ...
    def __show_introduction(self) -> None:
        ...
        print("Each guess consists of four letters corresponding to the colors listed above.")
        print("Example: to guess red, green, blue and orange, type rgbo and press Enter.")
        print()
    ...
    def __show_feedback(self, guess: str, code: str) -> None:
        unmatched_in_guess, unmatched_in_code = self.__get_unmatched_elements(guess, code)
        ...
        print("{:<60}{:>20}".format("Elements with the correct color and position:", correct_color_and_position))
        print("{:<60}{:>20}".format("Elements with the correct color but incorrect position:", correct_color))

    def __get_unmatched_elements(self, guess: str, code: str) -> Tuple[List[str], List[str]]:
        unmatched_in_guess = []
        unmatched_in_code = []
    ...
    def __ask_for_guess(self, guesses_left: int) -> str:
        guess = input(f"You have {guesses_left} guesses left. What's your guess? ").lower().strip()
        if not self.__is_valid_guess(guess):
            print("That's not a valid guess. Please enter four letters from an alphabet of", self.COLOR_LETTERS)
            guess = self.__ask_for_guess(guesses_left)
        return guess
